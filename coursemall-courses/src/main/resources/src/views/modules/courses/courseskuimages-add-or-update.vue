    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskuimages/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.courseSkuImages.skuId
                this.dataForm.imgUrl = data.courseSkuImages.imgUrl
                this.dataForm.imgSort = data.courseSkuImages.imgSort
                this.dataForm.defaultImg = data.courseSkuImages.defaultImg
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskuimages/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
