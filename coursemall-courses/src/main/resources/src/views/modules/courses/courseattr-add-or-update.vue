    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.attrId) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattr/info/${this.dataForm.attrId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.attrName = data.courseAttr.attrName
                this.dataForm.searchType = data.courseAttr.searchType
                this.dataForm.icon = data.courseAttr.icon
                this.dataForm.valueSelect = data.courseAttr.valueSelect
                this.dataForm.attrType = data.courseAttr.attrType
                this.dataForm.enable = data.courseAttr.enable
                this.dataForm.catelogId = data.courseAttr.catelogId
                this.dataForm.showDesc = data.courseAttr.showDesc
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattr/${!this.dataForm.attrId ? 'save' : 'update'}`),
              method: 'post',
