    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespucomment/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.courseSpuComment.skuId
                this.dataForm.spuId = data.courseSpuComment.spuId
                this.dataForm.spuName = data.courseSpuComment.spuName
                this.dataForm.memberNickName = data.courseSpuComment.memberNickName
                this.dataForm.star = data.courseSpuComment.star
                this.dataForm.memberIp = data.courseSpuComment.memberIp
                this.dataForm.createTime = data.courseSpuComment.createTime
                this.dataForm.showStatus = data.courseSpuComment.showStatus
                this.dataForm.spuAttributes = data.courseSpuComment.spuAttributes
                this.dataForm.likesCount = data.courseSpuComment.likesCount
                this.dataForm.replyCount = data.courseSpuComment.replyCount
                this.dataForm.resources = data.courseSpuComment.resources
                this.dataForm.content = data.courseSpuComment.content
                this.dataForm.memberIcon = data.courseSpuComment.memberIcon
                this.dataForm.commentType = data.courseSpuComment.commentType
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespucomment/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
