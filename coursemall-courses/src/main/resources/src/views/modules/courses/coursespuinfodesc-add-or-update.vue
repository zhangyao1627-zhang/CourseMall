    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.spuId) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuinfodesc/info/${this.dataForm.spuId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.decript = data.courseSpuInfoDesc.decript
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuinfodesc/${!this.dataForm.spuId ? 'save' : 'update'}`),
              method: 'post',
