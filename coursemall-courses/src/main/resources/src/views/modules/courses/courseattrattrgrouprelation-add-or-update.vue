    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattrattrgrouprelation/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.attrId = data.courseAttrAttrgroupRelation.attrId
                this.dataForm.attrGroupId = data.courseAttrAttrgroupRelation.attrGroupId
                this.dataForm.attrSort = data.courseAttrAttrgroupRelation.attrSort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattrattrgrouprelation/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
