    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskusaleattrvalue/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.courseSkuSaleAttrValue.skuId
                this.dataForm.attrId = data.courseSkuSaleAttrValue.attrId
                this.dataForm.attrName = data.courseSkuSaleAttrValue.attrName
                this.dataForm.attrValue = data.courseSkuSaleAttrValue.attrValue
                this.dataForm.attrSort = data.courseSkuSaleAttrValue.attrSort
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskusaleattrvalue/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
