    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.skuId) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskuinfo/info/${this.dataForm.skuId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.courseSkuInfo.spuId
                this.dataForm.skuName = data.courseSkuInfo.skuName
                this.dataForm.skuDesc = data.courseSkuInfo.skuDesc
                this.dataForm.catalogId = data.courseSkuInfo.catalogId
                this.dataForm.brandId = data.courseSkuInfo.brandId
                this.dataForm.skuDefaultImg = data.courseSkuInfo.skuDefaultImg
                this.dataForm.skuTitle = data.courseSkuInfo.skuTitle
                this.dataForm.skuSubtitle = data.courseSkuInfo.skuSubtitle
                this.dataForm.price = data.courseSkuInfo.price
                this.dataForm.saleCount = data.courseSkuInfo.saleCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseskuinfo/${!this.dataForm.skuId ? 'save' : 'update'}`),
              method: 'post',
