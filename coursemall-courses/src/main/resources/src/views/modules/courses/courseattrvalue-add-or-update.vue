    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattrvalue/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.courseAttrValue.spuId
                this.dataForm.attrId = data.courseAttrValue.attrId
                this.dataForm.attrName = data.courseAttrValue.attrName
                this.dataForm.attrValue = data.courseAttrValue.attrValue
                this.dataForm.attrSort = data.courseAttrValue.attrSort
                this.dataForm.quickShow = data.courseAttrValue.quickShow
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/courseattrvalue/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
