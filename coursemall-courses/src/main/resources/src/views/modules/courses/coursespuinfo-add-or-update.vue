    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuinfo/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuName = data.courseSpuInfo.spuName
                this.dataForm.spuDescription = data.courseSpuInfo.spuDescription
                this.dataForm.catalogId = data.courseSpuInfo.catalogId
                this.dataForm.brandId = data.courseSpuInfo.brandId
                this.dataForm.weight = data.courseSpuInfo.weight
                this.dataForm.publishStatus = data.courseSpuInfo.publishStatus
                this.dataForm.createTime = data.courseSpuInfo.createTime
                this.dataForm.updateTime = data.courseSpuInfo.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuinfo/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
