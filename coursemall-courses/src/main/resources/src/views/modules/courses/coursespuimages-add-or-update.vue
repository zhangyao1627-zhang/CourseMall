    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuimages/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.courseSpuImages.spuId
                this.dataForm.imgName = data.courseSpuImages.imgName
                this.dataForm.imgUrl = data.courseSpuImages.imgUrl
                this.dataForm.imgSort = data.courseSpuImages.imgSort
                this.dataForm.defaultImg = data.courseSpuImages.defaultImg
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursespuimages/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
