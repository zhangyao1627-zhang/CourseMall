    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.catId) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursecategory/info/${this.dataForm.catId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.courseCategory.name
                this.dataForm.parentCid = data.courseCategory.parentCid
                this.dataForm.catLevel = data.courseCategory.catLevel
                this.dataForm.showStatus = data.courseCategory.showStatus
                this.dataForm.sort = data.courseCategory.sort
                this.dataForm.icon = data.courseCategory.icon
                this.dataForm.productUnit = data.courseCategory.productUnit
                this.dataForm.productCount = data.courseCategory.productCount
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/courses/coursecategory/${!this.dataForm.catId ? 'save' : 'update'}`),
              method: 'post',
